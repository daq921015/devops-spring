...
public class BaoLiSwagger {
    ...
    public Docket createRestApi_baoli() {
        //公共参数列表
        Parameter appkey = new ParameterBuilder().name("appkey").defaultValue("123456").description("应用号。由汉朔提供").modelRef(new ModelRef("string")).parameterType("query").required(true).build();
        Parameter merchant_id = new ParameterBuilder().name("merchant_id").defaultValue("123456").description("商户ID:商家/平台渠道").modelRef(new ModelRef("string")).parameterType("query").required(true).build();
        ...
        Parameter sign = new ParameterBuilder().name("sign").defaultValue("aaaaa5aaaaa5aaaaa5aaaaa5aaaaa5aa").description("签名:32为MD5字符串").modelRef(new ModelRef("string")).parameterType("query").required(true).build();
        Parameter cw_ca_request = new ParameterBuilder().name("cw_ca_request").defaultValue("2").description("签名类型(1:证书签名2:其他密钥签名.如果为空,默认为2)").modelRef(new ModelRef("string")).parameterType("query").required(true).build();
